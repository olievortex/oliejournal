...
public interface IJournalProcess
{
    ...
    Task<PagedResultModel<JournalEntryListModel>> GetEntryList(string userId, int page, int pageSize, CancellationToken ct);
    Task<int> Ingest(string userId, Stream audio, float? latitude, float? longitude, string? ipAddress, ServiceBusSender sender, BlobContainerClient client, CancellationToken ct);
